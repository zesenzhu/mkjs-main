...
const Canvas: React.FC = () => {
    ...
    useEffect(() => {
        if (canvasRef.current) {
            const ctx = (ctxRef.current = canvasRef.current.getContext("2d")) as CanvasRenderingContext2D;
    ...
    const clearPalette = () => {
        ctxRef.current?.clearRect(0, 0, canvasRef.current?.width as number, canvasRef.current?.height as number)
    }
    return (
        <div style={{ height: '100%', width: '100%' }} >
            <Button className='handle' type="primary" onClick={clearPalette}   >清屏</Button>
            <InputNumber addonBefore={'线条宽度'} min={1} max={50} onChange={(value: any) => {
                if (ctxRef.current) ctxRef.current.lineWidth = value
            }} defaultValue={2} />
            <canvas ref={canvasRef}

                style={{ marginTop: 10, border: '1px #000 solid', height: '100%', width: '100%' }}></canvas>
        </div>
    );
};
